...
/*
 * With Adaptive Scedule extends HIS7
 */
public class HierarchicalIS8<SampleType> {
	...
	public HierarchicalIS8(int sample_num, int level,
	...
	private double getTargetBeta(int sampler,
		...
		double[] w = new double[partSamples.size()];

		double logZ_u = Math.log(sumW_u) + maxLogw_u-Math.log(partSamples.size());
		double logZ_l = Math.log(sumW_l) + maxLogw_l-Math.log(partSamples.size());
		
		for (int i = 0; i < partSamples.size(); i++) {
			w_u[i] = Math.exp(-partSamples.get(i).getValue()* inverseTemperature[sampler + 1]-logZ_u);
			w_l[i] = Math.exp(-partSamples.get(i).getValue()* inverseTemperature[sampler - 1]-logZ_l);
			w[i] = (w_u[i] - w_l[i]) / (w_u[i] + w_l[i]);
		}

		List<WeightedSampleWithValue<SampleType>> samples = generateMarginalizedSamples(
				sampler, partSamples, partSampler);
		double average = 0;
		double variance = 0;
		for (int i = 0; i < samples.size(); i++) {
			WeightedSampleWithValue<SampleType> sample = samples.get(i);
			average += w[i] * sample.getWeight();
		}
		for (int i = 0; i < samples.size(); i++) {
			WeightedSampleWithValue<SampleType> sample = samples.get(i);
			variance += (w[i] - average) * (w[i] - average)
					* sample.getWeight();
		}

		double beta = inverseTemperature[sampler] - average / (variance + 0.01);
		System.out.printf("%d: %f,%f->%f\n", sampler, average, variance, beta);
		return beta;
	}
