...
public class TruncationCE<SampleType> implements  Annealing<SampleType> {
	...
	public TruncationCE(int sample_num, CostFunction<SampleType> cf) {
		this.SAMPLE_NUM = sample_num;
		this.cf = cf;
	}
	
	public TruncationCE(int sample_num,double cutoff, CostFunction<SampleType> cf) {
		this.SAMPLE_NUM = sample_num;
		this.cf = cf;
		this.cutoff=cutoff;
	}
	...
	private List<WeightedSampleWithValue<SampleType>> generateMarginalizedSamples() {
		...
		for(int i=0;i<bestSamples.size();i++){
			logW[i]=-sampler.logNormalizedProbability(bestSamples.get(i).getSample());
		}
		LogSumExp.normalize(logW);
		for (int i = 0; i < bestSamples.size(); i++) {
			allSamples.add(new WeightedSampleWithValue<SampleType>(Math.exp(logW[i]),
					bestSamples.get(i)));
		}
		return allSamples;
	}
