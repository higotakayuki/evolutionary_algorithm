...
import optimize.optimizer.old.TruncationCEWithERS;

import statistics.sampler.MixtureSampler;

public class UMDATruncationCEWithERS extends TruncationCEWithERS<DVector> {
	public UMDATruncationCEWithERS(int sample_num, double cutoff,
		...
		for (int c = 0; c < 1; c++) {
			...
			try {
				...
				CostFunction<DVector> cf = new RecordFunction<DVector>(
						new TwoMax(dim), ps);
				UMDATruncationCEWithERS bumda = new UMDATruncationCEWithERS(
						200, 0.1, cf);
